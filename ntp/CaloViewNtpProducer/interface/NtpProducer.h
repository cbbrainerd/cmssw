...
/** \class NtpProducer2
 *
 * Creates ntuples defined in configuration file.
 *
 ...
 *
 * Example python config fragment to go with the class above. This dumps the pT, eta and phi variables (which are doubles), as well as the charges (which are ints):
 ...
 * Note that the FW automatically converts any function called to a double internally, so be sure that the return type of whatever is called is exactly representable as a double if it makes a difference
 */
